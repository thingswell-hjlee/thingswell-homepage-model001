#!/usr/bin/env bash
# ============================================================
# SafeGAI 홈페이지 - PRODUCTION 배포 스크립트 (S3 + CloudFront)
# 사전조건: AWS CLI 설치 + 자격증명(aws configure)
# 사용법:  cd grape && bash deploy-production.sh
# ============================================================
set -euo pipefail
...
echo "=========================================="
echo " PRODUCTION 배포 시작 ($EXPECTED_DOMAIN)"
echo "=========================================="
...
if ! grep -q "$EXPECTED_DOMAIN" dist/index.html; then
  echo "❌ dist/index.html 에서 '$EXPECTED_DOMAIN' 를 찾지 못함 — 빌드 모드/환경변수(.env.production) 확인. 중단."
  exit 1
fi
...
echo ""
echo "=========================================="
echo "✅ PRODUCTION 배포 완료: https://$EXPECTED_DOMAIN"
echo "=========================================="
